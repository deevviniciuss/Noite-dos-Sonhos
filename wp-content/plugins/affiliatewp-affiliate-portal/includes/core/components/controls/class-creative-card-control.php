...
final class Creative_Card_Control extends Base_Control {
	 ...
	 *         @type string           $image       Creative image to display on the card and on the preview. Default empty string.
	...
	public function render( $echo = true ) {
		...
		$content = "";
		if ( ! empty ( $args['image'] ) ) {
			$content = '<img class="object-scale-down h-48 w-full mx-auto" src="' . esc_attr( $args['image'] ) . '" alt="' . esc_attr( $args['text'] ) .'" />';
		} else {
	...
	private function get_modal( $args ) {
		...
		if ( ! empty ( $args['image'] ) ) {
			...
			$modal_data .= '<img class="" src="' . esc_attr( $args['image'] ) . '" alt="' . esc_attr( $args['text'] ) .'" />';
		...
		$image_or_text = '';
		if ( ! empty( $args['image'] ) ) {
			$image_or_text = '<img src="' . esc_attr( $args['image'] ) . '" alt="' . esc_attr( 'text' ) . '" />';
		} else {
