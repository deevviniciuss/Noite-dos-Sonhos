...
if ( ! class_exists( 'Smart_Manager_Base' ) ) {
	class Smart_Manager_Base {
													...
													FROM {$wpdb->prefix}term_relationships
														JOIN {$wpdb->prefix}term_taxonomy
															ON( {$wpdb->prefix}term_taxonomy.term_taxonomy_id = {$wpdb->prefix}term_relationships.term_taxonomy_id
																...
																". $join_condition ." ) 
														JOIN {$wpdb->prefix}terms
															ON ( {$wpdb->prefix}terms.term_id = {$wpdb->prefix}term_taxonomy.term_id ) ) as taxonomy_sort 
		...
		public function save_state() {
			...
			if( !empty( $slug ) && !empty( $this->req_params['dashboard_states'] ) && empty( $this->req_params['is_view'] ) ) {

				$dashboard_type = 'post_type';

				// Code to update the recent accessed dashboards
				sa_sm_update_recent_dashboards( 'post_types', $slug );
			...
			// Code to update the recent accessed dashboards
			if( ! empty( $slug ) && ! empty( $this->req_params['is_view'] ) ) {
				$dashboard_type = 'view';
				// Code to update the recent accessed dashboards
				sa_sm_update_recent_dashboards( 'views', $slug );
			}
